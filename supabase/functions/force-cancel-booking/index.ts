...
interface ForceCancelBookingPayload {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const payload = await parseJSON<ForceCancelBookingPayload>(request);
    ...
    // Analytics side-effect: track booking_force_cancelled
    try {
      ...
      await serviceClient.from("analytics_events").insert({
        event_type: "booking_force_cancelled",
        vendor_id: conv?.vendor_id ?? null,
        actor_id: user.id,
        metadata: {},
      });
    } catch (analyticsError) {
      console.warn("Analytics insert failed (force-cancel-booking):", analyticsError);
    }
