-- Messaging server functions for send-message and send-message-with-attachment edge functions
-- These RPCs are SECURITY DEFINER and restricted to service_role only.

--------------------------------------------------------------------------------
-- 1. send_message_server
--    Validates participant, enforces rate limit, deduplicates via client_id,
--    inserts the message, and returns the record as JSONB.
--    The increment_unread_on_message trigger handles unread counts and
--    last_activity_at automatically.
--------------------------------------------------------------------------------
...
BEGIN
  ...
  IF p_client_id IS NOT NULL THEN
    -- Try insert; ON CONFLICT does nothing if duplicate
    ...
    -- Fetch the row (either just inserted or previously existing)
    SELECT * INTO v_msg
    FROM messages
  ...
  -- 5. Return the message record as JSONB
...
--------------------------------------------------------------------------------
-- 2. send_message_with_attachment_server
--    Same as send_message_server plus inserts into message_attachments.
--    Returns message + attachment as JSONB.
--------------------------------------------------------------------------------
...
BEGIN
  ...
  -- 7. Return message + attachment as JSONB
  RETURN jsonb_build_object(
